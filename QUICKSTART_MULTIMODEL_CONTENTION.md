...
A run that completes is not the same as a run that measured something. Put these
to your agent once you have results — it has the manifests, the sampler traces
and the configs, and can answer each from them.

**Prompts:**
> "Is this benchmark representative? Check the GPU sampler in the manifests —
> was every tenant actually loaded, or was one saturated while the others idled?"

> "Is the workload really challenging these models? Look at the prompt lengths
> and output_tokens — what regime is this actually measuring?"

> "Is the serving configuration realistic for the offered rates, or is it tuned
> for load we never send?"

> "When two models are co-located, are they served by one vLLM server or two?"

> "Which vLLM parameters could change these results, and which would distort
