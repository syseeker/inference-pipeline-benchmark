...
requests/second, then collapse to **33-37x with 23% throughput loss** at 64.
A safe region with a flat tax, and a cliff. It also shows the study's
configured rates are low — `qwen2.5-7b` sustains 62 of 64 rps alone, so
`llm@4` is about 1/16th of capacity, and the Phase 3 and Phase 5 numbers
describe the lightly loaded regime.
...
# 4b. Derived Triton image, only if a colocation names a python-backend CV
#     model (kosmos-2.5: same-ilm, mix-ilm-cv, mix-vlm-ilm, mix-full). It runs
#     inside the container and imports torch + transformers, which the stock
#     tritonserver image does not ship — without this the tenant never loads.
...
8. **Confirm the video is actually being sent.** Run
   `cross-vlm-prefill-vs-llm` solo and check `http_req_data_sent` in the
   tenant's `profile_export.jsonl`: **~2,000 KB** per request for a 10s/720p
   clip, against ~0.1 KB for a text tenant. **Do NOT use
   `input_sequence_length`** — aiperf tokenizes the text only and never counts
   multimodal expansion, so it reports ~29 for a request whose true length is
   19,184 tokens (measured: vLLM rejected these exact requests at
   `--max-model-len=16384`). Reading ~29 as "video not sent" is the wrong
   conclusion from a correct run.
